...
export async function getNonce(api: TypedApi<typeof dev>, ss58Address: string): Promise<number> {
    const value = await api.query.System.Account.getValue(ss58Address);
    return value.nonce
}

export async function getNonceChangePromise(api: TypedApi<typeof dev>, ss58Address: string) {
...
export function convertPublicKeyToMultiAddress(publicKey: Uint8Array, ss58Format: number = 42): string {
...
export async function waitForFinalizedBlock(api: TypedApi<typeof dev>) {
...
export async function waitForTransactionCompletion2(api: TypedApi<typeof dev>, raw: Binary, signer: PolkadotSigner) {
    ...
    return new Promise<void>((resolve, reject) => {
        const subscription = tx.signSubmitAndWatch(signer).subscribe({
            next(value: { type: string; txHash: any; }) {
                console.log("Event:", value);
...
export async function waitForNonceChange(api: TypedApi<typeof dev>, ss58Address: string) {
