...
use crate::{
    ...
    StakeUnbondingLedgerV2, SubnetName, SubnetRemovalReason, SubnetsData,
    ...
    DelegateStakeCooldownEpochs, MaxUnbondings, SwapQueueOrder, SwapCallQueue, NextSwapId,
    QueuedSwapCall, QueuedSwapItem, MaxSubnets, MaxSubnetNodes, HotkeySubnetNodeId
};
...
fn insert_to_node_swap_call_queue(account_id: AccountIdOf<Test>, subnet_id: u32, subnet_node_id: u32, balance: u128) {
    let id = NextSwapId::<Test>::get();
...
fn test_update_swap_queue() {
  new_test_ext().execute_with(|| {
    ...
    let prev_total_subnet_delegate_stake_balance = TotalSubnetDelegateStakeBalance::<Test>::get(from_subnet_id);
    ...
    assert_ne!(prev_total_subnet_delegate_stake_balance, TotalSubnetDelegateStakeBalance::<Test>::get(from_subnet_id));
    assert!(prev_total_subnet_delegate_stake_balance > TotalSubnetDelegateStakeBalance::<Test>::get(from_subnet_id));
    ...
    assert!(queue.first().map_or(false, |&first_id| first_id == prev_next_id));
...
fn test_execute_queue() {
  new_test_ext().execute_with(|| {
    ...
    // let name_2: Vec<u8> = "subnet-name-2".into();
    // build_activated_subnet_new(name_2.clone(), 0, end, deposit_amount, stake_amount);
    // let subnet_id_2 = SubnetName::<Test>::get(name_2.clone()).unwrap();
    ...
    for n in 0..queues_count {
      ...
      if n % queues_count == 0 {
        ...
        // Sanity check
        let user_shares = AccountSubnetDelegateStakeShares::<Test>::get(&account(n), subnet_id_1);
        ...
        insert_to_node_swap_call_queue(account(n), subnet_id_1, hotkey_subnet_node_id.unwrap(), amount);
        let user_shares = AccountNodeDelegateStakeShares::<Test>::get((&account(n), subnet_id_1, hotkey_subnet_node_id.unwrap()));
        assert_eq!(user_shares, 0);
      }
    }
