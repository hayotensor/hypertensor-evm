...
use crate::{
    AccountSubnetDelegateStakeShares, TotalSubnetDelegateStakeBalance,
...
fn test_convert_to_shares_basic() {
    new_test_ext().execute_with(|| {
        ...
        let expected =
            U256::from(balance) * (U256::from(total_shares) + 10) / (U256::from(total_balance) + 1);
...
fn test_convert_to_balance_basic() {
    new_test_ext().execute_with(|| {
        ...
        let expected =
            U256::from(shares) * (U256::from(total_balance) + 1) / (U256::from(total_shares) + 10);
...
fn test_round_trip_conversion() {
    new_test_ext().execute_with(|| {
        ...
        let balance =
            Network::convert_to_balance(shares, total_shares + shares, total_balance + deposit);
...
fn test_convert_to_shares_and_balance_roundtrip_large_supply() {
    new_test_ext().execute_with(|| {
        ...
        Network::increase_account_delegate_stake(&account(1), subnet_id, user_balance, shares);
        ...
        assert!(
            user_balance > balance_back,
            "Balance back should be less than deposited balance"
        );
        ...
        assert!(
            real_shares < shares,
            "Real shares should be less than equated shares"
        );
    });
}
