...
fn test_deactivate_subnet_node_reactivate() {
    new_test_ext().execute_with(|| {
        ...
        let deactivated_subnet_node_data =
            DeactivatedSubnetNodesData::<Test>::try_get(subnet_id, subnet_node_id);
...
fn test_defer_node() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            subnet_epoch + registration_queue_epochs
        );
    })
}
...
fn test_clean_expired_registered_subnet_nodes_with_no_data() {
    new_test_ext().execute_with(|| {
        ...
        let queue_epochs = RegistrationQueueEpochs::<Test>::get(subnet_id_1);
...
fn test_clean_expired_registered_subnet_nodes_with_subnet_id() {
    new_test_ext().execute_with(|| {
        ...
        // Increase past registration epochs
        let queue_epochs = RegistrationQueueEpochs::<Test>::get(subnet_id);
        let grace_epochs = ActivationGraceEpochs::<Test>::get(subnet_id);
...
fn test_clean_expired_deactivated_subnet_nodes_with_no_data() {
    new_test_ext().execute_with(|| {
        ...
        // Subnet 1
        let prev_registered_count_1 =
            DeactivatedSubnetNodesData::<Test>::iter_prefix(subnet_id_1).count();
        assert_eq!(prev_registered_count_1 as u32, end);
        assert_ne!(prev_registered_count_1 as u32, 0);
        // Subnet 2
        let prev_registered_count_2 =
            DeactivatedSubnetNodesData::<Test>::iter_prefix(subnet_id_2).count();
...
fn test_clean_expired_deactivated_subnet_nodes_with_subnet_id() {
    new_test_ext().execute_with(|| {
        ...
        let prev_registered_count =
            DeactivatedSubnetNodesData::<Test>::iter_prefix(subnet_id).count();
        ...
        let registered_count = DeactivatedSubnetNodesData::<Test>::iter_prefix(subnet_id).count();
        assert_eq!(registered_count, 0);
    })
}
...
fn test_clean_expired_deactivated_subnet_nodes_with_subnet_node_id() {
    new_test_ext().execute_with(|| {
        ...
        assert_ne!(subnet_id, 0);
        assert_ne!(subnet_node_id, 0);
        assert!(DeactivatedSubnetNodesData::<Test>::try_get(subnet_id, subnet_node_id).is_ok());
