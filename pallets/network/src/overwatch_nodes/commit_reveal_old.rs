...
impl<T: Config> Pallet<T> {
  pub fn commit_ow_weights(
    ...
    mut commit_weights: Vec<OverwatchCommit>,
  ) -> DispatchResult {
    ...
    let total_commitments: u32 = commit_weights.len() as u32;
    ...
    SubnetBenchmarkCommitments::<T>::insert(epoch, hotkey, commit_weights);
  ...
  pub fn reveal_ow_weights(
    origin: T::RuntimeOrigin,
    mut reveal_weights: Vec<OverwatchReveal>,
    seed: &[u8],
  ) -> DispatchResult {
    let hotkey: T::AccountId = ensure_signed(origin)?;
    ...
    let reveal_weights_len = reveal_weights.len();

    ensure!(
      encrypted_weights.len() <= reveal_weights_len && reveal_weights_len > 0,
      Error::<T>::InvalidOverwatchCommitments
    );
    ...
    for commit in encrypted_weights.iter() {
      ...
      let revealed_weight: Option<&SubnetBenchmarkWeightReveal> = reveal_weights.iter().find(
  ...
  pub fn compute_ow_weights(epoch: u32) {
    let subnets: BTreeSet<_> = SubnetsData::<T>::iter().map(|(id, _)| id).collect();
