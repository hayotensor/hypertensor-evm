...
import { GENESIS_ACCOUNT, GENESIS_ACCOUNT_PRIVATE_KEY, GENESIS_ACCOUNT_BALANCE, EXISTENTIAL_DEPOSIT, ALL_ACCOUNTS, SEED_PATH, TEST_PATH } from "./config";
import { createAndFinalizeBlock, describeWithFrontier, customRequest, hash } from "./util";
import { getSubstrateApi } from "../helpers/substrate";
import { stringToBinary } from "../helpers/config";

// npm run test-script --PATH 'tests/test-hypertensor-subnet.ts'
...
describeWithFrontier("Hypertensor precompiles", (context) => {
  ...
  step("register subnet", async function () {
    ...
    // // const sharesAfter = await contract.methods.accountSubnetDelegateStakeShares(SEED_PATH);
