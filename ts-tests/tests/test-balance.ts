...
describeWithFrontier("Frontier RPC (Balance)", (context) => {
	...
	step("genesis balance is setup correctly", async function () {
		...
		expect(await context.web3.eth.getBalance(GENESIS_ACCOUNT)).to.equal((BigInt(GENESIS_ACCOUNT_BALANCE) - BigInt(EXISTENTIAL_DEPOSIT)).toString());
	});
	...
	step("balance to be updated after transfer", async function () {
		...
		// ).toString();
		const expectedGenesisBalance = (
			BigInt(GENESIS_ACCOUNT_BALANCE) - BigInt(EXISTENTIAL_DEPOSIT) -
			BigInt(21000) * BigInt(GAS_PRICE) -
			BigInt(TRANFER_VALUE)
		).toString();
