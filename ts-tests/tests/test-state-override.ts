...
describeWithFrontier("Frontier RPC (StateOverride)", (context) => {
	...
	it("should have a sender balance of 4500 with state override and EXISTENTIAL_DEPOSIT", async function () {
		const { result } = await customRequest(context.web3, "eth_call", [
			{
				from: GENESIS_ACCOUNT,
				to: contractAddress,
		...
		expect(Web3.utils.hexToNumberString(result)).to.equal("4500");
	});
